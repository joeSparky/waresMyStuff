...
public class SearchOffsetTest {
	...
	public void testGetNewOffsetInitialValues() {
		...
		if (so.getOffset(IdAndStrings.DIRECTION.FORWARD, true) != 0)
			fail("expected and offset of zero");
		so = new SearchOffset();
		if (so.getOffset(IdAndStrings.DIRECTION.REVERSE, true) != 0)
			fail("expected and offset of zero");
	}
	...
	public void testGetNewOffsetForward() {
		...
		if (so.getOffset(IdAndStrings.DIRECTION.FORWARD, true) != IdAndStrings.DISPLAYSIZE)
	...
	public void testGetNewOffsetReverse() {
		...
		so.storeLastSearchResults(IdAndStrings.DIRECTION.FORWARD, IdAndStrings.DISPLAYSIZE);
		if (so.getOffset(IdAndStrings.DIRECTION.FORWARD, true) != IdAndStrings.DISPLAYSIZE*2)
			fail("expected an offset of DISPLAYSIZE * 2");
		
		// ask for the offset if we were to go backwards
		if (so.getOffset(IdAndStrings.DIRECTION.REVERSE, true) != IdAndStrings.DISPLAYSIZE)
	...
	public void testShowNextButton() {
		...
		so.getOffset(IdAndStrings.DIRECTION.FORWARD, true);
